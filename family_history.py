...
def print_death_age(people_dict):
    ...
    for i in people_dict:
        print(f"Name: {people_dict[i][NAME_INDEX]}\nDeath at age: {people_dict[i][DEATH_YEAR_INDEX] - people_dict[i][BIRTH_YEAR_INDEX]}")
...
def count_genders(people_dict):
    ...
    for person_key, person_data in people_dict.items():
        gender = person_data[1]
        if gender == 'F':
            count_F += 1
        elif gender == 'M':
            count_M += 1
        else:
            print('The Count_genders function is not working')
    print(f'There are {count_F} females and {count_M} males.')
...
def print_marriages(marriages_dict, people_dict):
    ...
    """
    """for i in people_dict:
        for e in marriages_dict:
            if people_dict[i][NAME_INDEX] == marriages_dict[e][HUSBAND_KEY_INDEX]:
                print(f"Name: {people_dict[i][HUSBAND_KEY_INDEX]}\nMarried at age: {marriages_dict[e][WEDDING_YEAR_INDEX] - people_dict[i][BIRTH_YEAR_INDEX]}")
"""
    for i in marriages_dict:
        husband = marriages_dict[i][HUSBAND_KEY_INDEX]
        wife = marriages_dict[i][WIFE_KEY_INDEX]
        year = marriages_dict[i][WEDDING_YEAR_INDEX]
        husbandName = people_dict[husband][NAME_INDEX]
        wifeName = people_dict[wife][NAME_INDEX]
        husbandAge = year - people_dict[husband][BIRTH_YEAR_INDEX]
        wifeAge = year - people_dict[wife][BIRTH_YEAR_INDEX]
        print(f"{husbandName} was {husbandAge} years old when he married {wifeName} who was {wifeAge} years old in {year}.")
