...
def main():
    ...
    # Prompt the user for a filename and open that text file.
        ...
        with open(filename, "rt") as text_file:
            ...
            perc_reduc = get_user_input("Percentage reduction in using a cell phone while driving: ", 0, 100)
        ...
        print()
        print("Bad input. Please enter a valid number (0-100).")

    except (csv.Error, KeyError) as csv_err:
        # This code will be executed if the CSV file
        # contains an error or if the CSV file is missing
        # a column heading that this program needs.
        print()
        print("The CSV file is formatted incorrectly.")

    except ZeroDivisionError as divide_zero_err:
        # This code will be executed if the denominator
        # in the estimate_reduction function is zero.
        print()
        print("Cannot divide by zero.")

def get_user_input(prompt, lowBound, HighBound):
    """Prompt the user for a number and return the number
    if it is within the specified range.

    Parameters
        input: The prompt to display to the user
        lowBound: The smallest number that the user can enter
        HighBound: The largest number that the user can enter
    ...
    while number == None:
        try:
            ...
            if lowBound <= number <= HighBound:
                return number
            else:
                print(f"Please enter a number between {lowBound} and {HighBound}.")
                number = None
        except ValueError:
